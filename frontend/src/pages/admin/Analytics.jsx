...
import { RefreshCw, Eye, ShoppingBag, TrendingUp, Percent, Download, Calendar, Landmark } from "lucide-react";
...
export default function AdminAnalytics() {
  ...
  const buildUrl = () => {
    if (range === "custom") {
      return `/orders/admin/analytics/?from=${fromDate}&to=${toDate}`;
    }
    return `/orders/admin/analytics/?range=${range}`;
  };

  const buildExportUrl = () => {
    if (range === "custom") {
      return `${API_BASE}/orders/admin/analytics/export/?from=${fromDate}&to=${toDate}`;
    }
    return `${API_BASE}/orders/admin/analytics/export/?range=${range}`;
  };
  ...
  return (
    <div className="p-6">
      ...
      {!data ? (
        ...
        <>
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <StatCard icon={TrendingUp} label="Gross Revenue" value={formatPKR(data.platform_revenue)} hint={`${data.order_count} order${data.order_count !== 1 ? "s" : ""}`} />
            <StatCard icon={Landmark} label="Platform Commission" value={formatPKR(data.platform_commission)} />
          ...
          <p className="mt-4 text-xs text-gray-400">
            ...
            For the always-on month-over-month KPI view, see the Dashboard.
          </p>
        </>
      )}
    </div>
